@if(count($importBills) > 0)
    @foreach($importBills as $processedBill)
        @php
        ...
        <div style="page-break-after: always;"> <!-- Add page break for printing -->
            ...
            <h3>SUB: CLEARING BILL FOR {{ $bill->item ?? 'N/A' }}</h3>

            <table class="info-table">
                <tr>
                    <td class="info-key">L/C No</td>
                    <td class="info-value">{{ $bill->lc_no ?? 'N/A' }}</td>
                    <td class="info-key">Date</td>
                    <td class="info-value">{{ $bill->lc_date?->format('d/m/Y') ?? 'N/A' }}</td>
                </tr>
                <tr>
                    <td class="info-key">ITEM</td>
                    <td class="info-value">{{ $bill->item ?? 'N/A' }}</td>
                    <td class="info-key">VALUE</td>
                    <td class="info-value">{{ number_format($bill->value ?? 0, 2) }}</td>
                </tr>
                <tr>
                    <td class="info-key">QTY</td>
                    <td class="info-value">{{ $bill->qty ?? 'N/A' }}</td>
                    <td class="info-key">WEIGHT</td>
                    <td class="info-value">{{ $bill->weight ?? 'N/A' }}</td>
                </tr>
                <tr>
...
@else
    <div class="alert alert-info text-center">
        No import bills found for the selected criteria.
    </div>
@endif
